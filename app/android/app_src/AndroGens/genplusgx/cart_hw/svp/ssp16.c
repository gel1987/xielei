...
// 0
#define rX     ssp->gr[SSP_X].u.h
#define rY     ssp->gr[SSP_Y].u.h
#define rA     ssp->gr[SSP_A].u.h
#define rST    ssp->gr[SSP_ST].u.h  // 4
#define rSTACK ssp->gr[SSP_STACK].u.h
#define rPC    ssp->gr[SSP_PC].u.h
#define rP     ssp->gr[SSP_P]
#define rPM0   ssp->gr[SSP_PM0].u.h  // 8
#define rPM1   ssp->gr[SSP_PM1].u.h
#define rPM2   ssp->gr[SSP_PM2].u.h
#define rXST   ssp->gr[SSP_XST].u.h
#define rPM4   ssp->gr[SSP_PM4].u.h  // 12
// 13
#define rPMC   ssp->gr[SSP_PMC]    // will keep addr in .h, mode in .l
#define rAL    ssp->gr[SSP_A].u.l

#define rA32   ssp->gr[SSP_A].v
#define rIJ    ssp->reg.r
...
#define REG_READ(r) (((r) <= 4) ? ssp->gr[r].u.h : read_handlers[r]())
#define REG_WRITE(r,d) { \
  int r1 = r; \
  if (r1 >= 4) write_handlers[r1](d); \
  else if (r1 > 0) ssp->gr[r1].u.h = d; \
}
...
static u32 read_PMC(void)
{
  ...
  if (ssp->emu_status & SSP_PMC_HAVE_ADDR) {
    ...
    return ((rPMC.u.l << 4) & 0xfff0) | ((rPMC.u.l >> 4) & 0xf);
  } else {
    ssp->emu_status |= SSP_PMC_HAVE_ADDR;
    //return rPMC.h;
    return rPMC.u.l;
...
static void write_PMC(u32 d)
{
  if (ssp->emu_status & SSP_PMC_HAVE_ADDR) {
    ...
    rPMC.u.h = d;
#ifdef LOG_SVP
    elprintf(EL_SVP, "PMC w m %04x @ %04x", rPMC.u.l, GET_PPC_OFFS());
#endif
  } else {
    ssp->emu_status |= SSP_PMC_HAVE_ADDR;
    //rPMC.h = d;
    rPMC.u.l = d;
#ifdef LOG_SVP
    elprintf(EL_SVP, "PMC w a %04x @ %04x", rPMC.u.h, GET_PPC_OFFS());
#endif
  }
}
...
static u32 ptr1_read_(int ri, int isj2, int modi3)
{
  ...
  switch (t)
  {
    ...
    case 0x00:
    case 0x01:
    case 0x02: return ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]];
    case 0x03: return ssp->ram.u.RAM0[0];
    case 0x04:
    case 0x05:
    case 0x06: return ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]];
    case 0x07: return ssp->ram.u.RAM1[0];
    // mod=1 (01), "+!"
    case 0x08:
    case 0x09:
    case 0x0a: return ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]++];
    case 0x0b: return ssp->ram.u.RAM0[1];
    case 0x0c:
    case 0x0d:
    case 0x0e: return ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]++];
    case 0x0f: return ssp->ram.u.RAM1[1];
    // mod=2 (10), "-"
    case 0x10:
    case 0x11:
    case 0x12: rp = &ssp->reg.u.r0[t&3]; t = ssp->ram.u.RAM0[*rp];
               if (!(rST&7)) { (*rp)--; return t; }
               add = -1; goto modulo;
    case 0x13: return ssp->ram.u.RAM0[2];
    case 0x14:
    case 0x15:
    case 0x16: rp = &ssp->reg.u.r1[t&3]; t = ssp->ram.u.RAM1[*rp];
               if (!(rST&7)) { (*rp)--; return t; }
               add = -1; goto modulo;
    case 0x17: return ssp->ram.u.RAM1[2];
    // mod=3 (11), "+"
    case 0x18:
    case 0x19:
    case 0x1a: rp = &ssp->reg.u.r0[t&3]; t = ssp->ram.u.RAM0[*rp];
               if (!(rST&7)) { (*rp)++; return t; }
               add = 1; goto modulo;
    case 0x1b: return ssp->ram.u.RAM0[3];
    case 0x1c:
    case 0x1d:
    case 0x1e: rp = &ssp->reg.u.r1[t&3]; t = ssp->ram.u.RAM1[*rp];
               if (!(rST&7)) { (*rp)++; return t; }
               add = 1; goto modulo;
    case 0x1f: return ssp->ram.u.RAM1[3];
  }
...
static void ptr1_write(int op, u32 d)
{
  ...
  switch (t)
  {
    ...
    case 0x00:
    case 0x01:
    case 0x02: ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]] = d; return;
    case 0x03: ssp->ram.u.RAM0[0] = d; return;
    case 0x04:
    case 0x05:
    case 0x06: ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]] = d; return;
    case 0x07: ssp->ram.u.RAM1[0] = d; return;
    // mod=1 (01), "+!"
    // mod=3,      "+"
    case 0x08:
    case 0x18:
    ...
    case 0x19:
    case 0x0a:
    case 0x1a: ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]++] = d; return;
    case 0x0b: ssp->ram.u.RAM0[1] = d; return;
    case 0x0c:
    case 0x1c:
    case 0x0d:
    case 0x1d:
    case 0x0e:
    case 0x1e: ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]++] = d; return;
    case 0x0f: ssp->ram.u.RAM1[1] = d; return;
    // mod=2 (10), "-"
    case 0x10:
    case 0x11:
    case 0x12: ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]--] = d; return;
    case 0x13: ssp->ram.u.RAM0[2] = d; return;
    case 0x14:
    case 0x15:
    case 0x16: ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]--] = d; return;
    case 0x17: ssp->ram.u.RAM1[2] = d; return;
    // mod=3 (11)
    case 0x1b: ssp->ram.u.RAM0[3] = d; return;
    case 0x1f: ssp->ram.u.RAM1[3] = d; return;
  }
}
...
static u32 ptr2_read(int op)
{
  ...
  switch (t)
  {
    ...
    case 0x00:
    case 0x01:
    case 0x02: mv = ssp->ram.u.RAM0[ssp->reg.u.r0[t&3]]++; break;
    case 0x03: mv = ssp->ram.u.RAM0[0]++; break;
    case 0x04:
    case 0x05:
    case 0x06: mv = ssp->ram.u.RAM1[ssp->reg.u.r1[t&3]]++; break;
    case 0x07: mv = ssp->ram.u.RAM1[0]++; break;
    // mod=1 (01)
    case 0x0b: mv = ssp->ram.u.RAM0[1]++; break;
    case 0x0f: mv = ssp->ram.u.RAM1[1]++; break;
    // mod=2 (10)
    case 0x13: mv = ssp->ram.u.RAM0[2]++; break;
    case 0x17: mv = ssp->ram.u.RAM1[2]++; break;
    // mod=3 (11)
    case 0x1b: mv = ssp->ram.u.RAM0[3]++; break;
    case 0x1f: mv = ssp->ram.u.RAM1[3]++; break;
...
void ssp1601_run(int cycles)
{
  ...
  while (g_cycles > 0 && !(ssp->emu_status & SSP_WAIT_MASK))
  {
    ...
    switch (op >> 9)
    {
      ...
      // ld adr, a
      case 0x07: ssp->ram.RAM[op & 0x1ff] = rA; break;
      ...
      // OP a, adr
      case 0x03: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_LDA (tmpv); break;
      case 0x13: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_SUBA(tmpv); break;
      case 0x33: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_CMPA(tmpv); break;
      case 0x43: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_ADDA(tmpv); break;
      case 0x53: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_ANDA(tmpv); break;
      case 0x63: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_ORA (tmpv); break;
      case 0x73: tmpv = ssp->ram.RAM[op & 0x1ff]; OP_EORA(tmpv); break;
